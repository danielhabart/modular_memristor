...
class MemristorModel:
    ...
    def kernel(self, t:np.ndarray) -> np.ndarray:
        """
        Returns impulse response k(t) sampled on same time grid as input.
        Replace with your volatility kernel.
        """
        p = self.params
        if t > 0: return 1/np.power(t+p.epsilon,p.alpha+1)
        else: return 0
    ...
    def evaluate(self, t:np.ndarray,v:np.ndarray, initState= ModelState) -> Dict[str,Any]:
        """
        Batch evaluation. Input: time, voltage, initial state. Output 
